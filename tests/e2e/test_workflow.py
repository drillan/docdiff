...
""")
    ...
    def test_full_workflow_cli(self, temp_project):
        ...
        try:
            ...
            result = runner.invoke(app, ["parse", str(temp_project / "docs" / "en")])
            assert result.exit_code == 0
            # Check for successful parse by looking for output indications
            assert (
                "Database saved" in result.output or "nodes extracted" in result.output
            )

            # Step 2: Parse target documents
            result = runner.invoke(app, ["parse", str(temp_project / "docs" / "ja")])
            assert result.exit_code == 0
    ...
    def test_workflow_with_api(self, temp_project, cache_manager):
        ...
        # Check that we have some comparison results
        assert len(result.mappings) > 0
        assert len(result.mappings) == len(source_nodes)

        # Check mapping types
        mapping_types = [m.mapping_type for m in result.mappings]
        # Since target has partial translations, we expect some missing
        assert "missing" in mapping_types or len(mapping_types) > 0
    ...
    def test_error_recovery(self, temp_project):
        ...
        result = runner.invoke(
            app, ["compare", "/non/existent/path", "/another/bad/path"]
        )
        assert result.exit_code != 0
        ...
        result = runner.invoke(
            app, ["import", str(bad_json), str(temp_project / "docs" / "ja")]
        )
        assert result.exit_code != 0
