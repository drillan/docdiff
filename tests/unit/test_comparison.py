...
from docdiff.models import DocumentNode, NodeType, TranslationStatus
...
class TestComparisonResult:
    ...
    def test_generate_html_report(self, sample_source_nodes, sample_target_nodes):
        ...
        result = ComparisonResult(
            structure_diff={'section': {'source': 1, 'target': 1, 'diff': 0, 'coverage': 100.0}},
            content_changes=[],
            translation_pairs=[],
...
class TestComparisonEngine:
    ...
    def test_are_corresponding_files(self, sample_source_nodes, sample_target_nodes):
        ...
        assert engine._are_corresponding_files(
        ...
        # Different files
        node1 = DocumentNode(
            id="1", type=NodeType.SECTION, content="",
            file_path=Path("docs/en/guide.md"),
            line_number=1, content_hash=""
        )
        node2 = DocumentNode(
            id="2", type=NodeType.SECTION, content="",
            file_path=Path("docs/ja/index.md"),
            line_number=1, content_hash=""
        )
    ...
    def test_create_node_mappings(self, sample_source_nodes, sample_target_nodes):
        ...
        engine = ComparisonEngine()
        mappings = engine._create_node_mappings(sample_source_nodes, sample_target_nodes)
    ...
    def test_calculate_coverage(self, sample_source_nodes, sample_target_nodes):
        ...
        assert coverage['counts']['missing'] == 1  # Code block is missing
        assert 0.6 <= coverage['overall'] <= 0.7  # ~67% coverage
