...
class TestTranslationDetection:
    ...
    def test_multiple_nodes_mixed_translation_status(self):
        ...
        assert result.coverage_stats['counts']['translated'] == 2  # Two sections translated
        assert result.coverage_stats['counts']['missing'] == 1  # One paragraph not translated
    ...
    def test_no_structural_match_uses_content_matching(self):
        ...
        if mapping.target_node and mapping.target_node.content == "これはラベルのない段落です。":
            assert mapping.mapping_type in ['exact', 'fuzzy']
        else:
