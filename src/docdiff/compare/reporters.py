...
class MarkdownReporter:
    ...
    def _generate_detailed(self, result: ComparisonResult, include_badges: bool = False) -> str:
        ...
        total = result.coverage_stats['counts']['total']
        translated = result.coverage_stats['counts']['translated']
        missing = result.coverage_stats['counts']['missing']
        fuzzy = result.coverage_stats['counts'].get('fuzzy', 0)
        ...
        lines.append(f"| **Overall Coverage** | {coverage_pct:.1f}% | {self._get_progress_bar(coverage_pct)} |")
        lines.append("")
    ...
    def _generate_compact(self, result: ComparisonResult) -> str:
        ...
        lines.append(f"## Translation Status: {coverage_pct:.1f}% {self._get_coverage_emoji(coverage_pct)}")
        lines.append("")
        lines.append(f"**{missing}/{total}** translations needed")
        lines.append("")
        
        # Top missing items
        missing_mappings = [m for m in result.mappings if m.mapping_type == 'missing'][:5]
        if missing_mappings:
            lines.append("### Top Missing:")
            for m in missing_mappings:
                lines.append(f"- [ ] `{m.source_node.label or m.source_node.name or 'node'}`: {m.source_node.content[:50]}...")
        
        return "\n".join(lines)
    
    def _generate_github(self, result: ComparisonResult, include_badges: bool = True) -> str:
        ...
        if include_badges:
            lines.append(f"![Coverage](https://img.shields.io/badge/coverage-{coverage_pct:.1f}%25-{self._get_badge_color(coverage_pct)})")
            lines.append(f"![Language](https://img.shields.io/badge/lang-{result.source_lang}→{result.target_lang}-blue)")
            lines.append("")
    ...
    def _generate_metadata_section(self, result: ComparisonResult) -> List[str]:
        ...
        if label_stats:
            ...
            # Sort by coverage
            sorted_stats = sorted(label_stats.items(), key=lambda x: x[1][2], reverse=True)
        ...
        if name_stats:
            ...
            sorted_stats = sorted(name_stats.items(), key=lambda x: x[1][2], reverse=True)
            
            for name, (translated, total, coverage) in sorted_stats[:10]:
                emoji = self._get_coverage_emoji(coverage)
                lines.append(f"- {emoji} `@{name}` - {translated}/{total} ({coverage:.0f}%)")
    ...
    def _generate_sidebyside_section(self, result: ComparisonResult) -> List[str]:
        ...
        for mapping in result.mappings[:20]:
            ...
            lines.append(f"| {status} | {source} | {target} | {match} | {metadata_str} |")
    ...
    def _generate_missing_section(self, result: ComparisonResult) -> List[str]:
        ...
        for file_name, mappings in list(by_file.items())[:5]:
            ...
            for m in mappings[:5]:
                ...
                lines.append(f"  ```")
                lines.append(f"  {node.content[:100]}{'...' if len(node.content) > 100 else ''}")
                lines.append(f"  ```")
    ...
    def _generate_stats_table(self, result: ComparisonResult) -> List[str]:
        ...
        lines.append("|:-------|------:|-----------:|")
        lines.append(f"| Total | {stats['counts']['total']} | 100% |")
        lines.append(f"| Translated | {stats['counts']['translated']} | {stats.get('exact_match', 0)*100:.1f}% |")
        lines.append(f"| Fuzzy | {stats['counts'].get('fuzzy', 0)} | {stats.get('fuzzy_match', 0)*100:.1f}% |")
        lines.append(f"| Missing | {stats['counts']['missing']} | {stats.get('missing', 0)*100:.1f}% |")
        lines.append("")
    ...
    def _calculate_metadata_stats(self, mappings: List[NodeMapping], attr: str) -> Dict[str, tuple]:
    ...
    def _group_missing_by_metadata(self, result: ComparisonResult) -> Dict[str, List[NodeMapping]]:
