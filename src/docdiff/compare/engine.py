...
class ComparisonEngine:
    ...
    def compare(
        ...
        translation_pairs = self._create_translation_pairs(mappings, source_lang, target_lang)
    ...
    def _create_node_mappings(
        ...
        for source_node in source_nodes:
            # First, try to find structurally corresponding node
            best_target = None
            best_similarity = 0.0
    ...
    def _find_exact_match(
        ...
        if source_node.label:
            ...
            if key in target_map and target_map[key].id not in used_targets:
                ...
                # Found structural match, but need to check if actually translated
                # Return None here to force content comparison in fuzzy matching
                return None
        ...
        if hasattr(source_node, 'file_path') and source_node.file_path:
            ...
            rel_path = Path(*source_node.file_path.parts[2:]) if len(source_node.file_path.parts) > 2 else source_node.file_path
    ...
    def _are_corresponding_files(self, node1: DocumentNode, node2: DocumentNode) -> bool:
        ...
        file1 = Path(*node1.file_path.parts[2:]) if len(node1.file_path.parts) > 2 else node1.file_path.name
        file2 = Path(*node2.file_path.parts[2:]) if len(node2.file_path.parts) > 2 else node2.file_path.name
    ...
    def _build_node_index(self, nodes: List[DocumentNode]) -> Dict[str, DocumentNode]:
        ...
        for node in nodes:
            ...
            if hasattr(node, 'file_path') and node.file_path:
                rel_path = Path(*node.file_path.parts[2:]) if len(node.file_path.parts) > 2 else node.file_path
    ...
    def _calculate_coverage(self, mappings: List[NodeMapping]) -> Dict[str, float]:
        """Calculate translation coverage statistics."""
        total = len(mappings)
        if total == 0:
            return {'overall': 0.0, 'counts': {'total': 0}}
    ...
    def _detect_content_changes(self, mappings: List[NodeMapping]) -> List[Dict[str, Any]]:
        ...
        for mapping in mappings:
            if mapping.target_node and 0 < mapping.similarity < 1.0:
                change = {
                    ...
                    'file': str(mapping.source_node.file_path) if hasattr(mapping.source_node, 'file_path') else 'unknown',
                    ...
                    'source_preview': mapping.source_node.content[:100] if mapping.source_node.content else '',
                    'target_preview': mapping.target_node.content[:100] if mapping.target_node.content else ''
                }
