...
def import_command(
    ...
    if format is None:
        ...
        if suffix == '.json':
            ...
            console.print(f"[red]Error: Cannot detect format from extension '{suffix}'. Please specify --format[/red]")
            raise typer.Exit(1)
    ...
    with Progress(
        ...
        task = progress.add_task(f"Loading existing {target_lang} documents...", total=None)
        parser = MySTParser()
        target_nodes = []
        ...
        # Import translations
        task = progress.add_task(f"Importing translations from {format.upper()}...", total=None)
