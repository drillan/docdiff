...
def compare_command(
    ...
    if view == "tree":
        ...
        table.add_row("Overall Coverage", f"{overall:.1%}")
        table.add_row("Total Nodes", str(counts["total"]))
        table.add_row("Translated", str(counts["translated"]))
        table.add_row("Missing", str(counts["missing"]))
        table.add_row("Fuzzy Matches", str(counts.get("fuzzy", 0)))
    ...
    if result.structure_diff:
        ...
        for node_type, stats in result.structure_diff.items():
            ...
            struct_table.add_row(
                node_type, str(stats["source"]), str(stats["target"]), diff_str
            )
