...
class TranslationImporter:
            ...
            for row in reader:
                ...
                translations.append({
                    ...
                    'context': {
                        'label': row.get('Label', ''),
                        'name': row.get('Name', '')
                    } if row.get('Label') or row.get('Name') else {}
                })
    ...
    def _import_xliff(
        ...
        try:
            ...
            raise ImportError("lxml is required for XLIFF import. Install with: pip install lxml")
        ...
        for file_elem in root.xpath('//xliff:file', namespaces=nsmap):
            ...
            for unit in file_elem.xpath('.//xliff:unit', namespaces=nsmap):
                ...
                for segment in segments:
                    ...
                    source_text = source_elem[0].text if source_elem and source_elem[0].text else ''
                    target_text = target_elem[0].text if target_elem and target_elem[0].text else ''
    ...
    def _create_node_from_translation(
        ...
        node = DocumentNode(
            ...
            line_number=translation.get('line', 0) if translation.get('line') is not None else 0
        )
            ...
            if source_id in trans_index:
                ...
                if trans['target']:
                    ...
                    if not mapping.target_node:
                        ...
                        mapping.target_node.content_hash = self._calculate_hash(trans['target'])
